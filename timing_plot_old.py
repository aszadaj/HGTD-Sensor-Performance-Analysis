...
def produceTimingDistributionPlots(time_difference, peak_value, peak_time, numberOfEventPerRun):
    ...
    for chan in channels:
        if chan != SiPM_chan:
            ...
            # Perform the fit
            time_diff[chan].Fit("gaus", "Q0", "", xMin, xMax)
            ...
            yAxisTitle = "Number (N)"
            fileName = dm.getSourceFolderPath() + "plots_hgtd_efficiency_sep_2017/"+md.getNameOfSensor(chan)+"/timing/timing_distribution/timing_distribution_"+str(md.getBatchNumber())+"_"+chan+ "_"+str(md.getNameOfSensor(chan))+".pdf"
